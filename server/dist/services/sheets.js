...
async function readTab(tab, useCache = true) {
...
async function getSheetGid(tab) {
    ...
    for (const s of res.data.sheets ?? []) {
        ...
        if (p?.title && p.sheetId != null)
            gidCache.set(p.title, p.sheetId);
    }
    const gid = gidCache.get(tab);
    if (gid == null)
        throw new Error(`Pestaña no encontrada en la hoja: ${tab}`);
    return gid;
}
// ===== Fechas =====
...
// ===== Escrituras =====
async function updateRowCells(tab, rowNumber, headers, cellUpdates) {
...
async function appendRow(tab, headers, rowValues) {
...
function mapRowToLead(row, idColumn) {
    const v = row.values;
    const id = v[idColumn] ?? '';
    return {
        id,
        ...
        notas: v['Notas'] ?? '',
        estado: (v['Estado'] || 'Sin iniciar'),
        agencias: v['Agencias'] ?? '',
    };
}
function camelToCellUpdates(updates) {
    const mapped = {};
...
export async function getLeads(filters) {
    ...
    if (filters?.search) {
        ...
        leads = leads.filter((l) => l.nombre.toLowerCase().includes(s) ||
...
export async function getLead(id) {
    ...
    lead.actividades = entries.map((h) => ({
        id: h.id,
        tipo: 'email',
...
export async function getAgenciasBase(search) {
    const { rows } = await readTab(TAB_AGENCIAS_BASE);
    let agencias = rows.map((r) => mapRowToLead(r, 'ID'));
    if (search) {
        const s = search.toLowerCase();
        agencias = agencias.filter((l) => l.nombre.toLowerCase().includes(s) ||
            l.email.toLowerCase().includes(s) ||
            l.provincia.toLowerCase().includes(s));
    }
    return agencias;
}
export async function exportAgenciasFromBase(sourceIds) {
    const failed = [];
    let created = 0;
    const base = await readTab(TAB_AGENCIAS_BASE, false);
    const agencias = await readTab(TAB_AGENCIAS, false);
    let nextIdag = agencias.rows.reduce((max, r) => Math.max(max, Number(r.values['idag']) || 0), 0) + 1;
    const exportedRowNumbers = [];
...
// ===== Historial y Copys =====
export async function getHistorial(idag) {
...
let usuariosReady = null;
function ensureUsuariosTab() {
    if (!usuariosReady) {
        usuariosReady = (async () => {
            const res = await sheets.spreadsheets.get({
                spreadsheetId: SPREADSHEET_ID,
...
export async function createUser(data) {
    ...
    const nextId = rows.reduce((max, r) => Math.max(max, Number(r.values['ID']) || 0), 0) + 1;
...
export async function updateUser(id, updates) {
    ...
    for (const [key, value] of Object.entries(updates)) {
        ...
        if (column)
            cellUpdates[column] = String(value ?? '');
    }
    const { headers, rows } = await readTab(TAB_USUARIOS, false);
    const row = rows.find((r) => (r.values['ID'] ?? '') === id);
    if (!row) {
